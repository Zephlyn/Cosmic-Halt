...
namespace CosmicSpell {
    ...
    static class FreezeDataExtension {
        public static CreatureFreezeData FreezeData(this Creature creature) {
            return creature?.gameObject.GetOrAddComponent<CreatureFreezeData>();
        }
    }
    ...
    public class CosmicSpellCharge : SpellCastCharge {
        ...
        private List<Creature> previousCreatures = new List<Creature>();
        ...
        public void PullItemToPoint(Item item, Vector3 target, float forceMultiplier = 1) {
            ...
            if (forceDirection.magnitude < 0.7f) {
                if (item.itemId.StartsWith("BlasterBolt") || item.itemId.StartsWith("FisherElementalProjectile")) {
        ...
        public IEnumerable<Creature> GetCreatureListInRadius(Vector3 origin, float radius) {
            return Creature.list.Where(
                    creature => Vector3.Distance(creature.transform.position, origin) < radius
        ...
        public override void UpdateCaster() {
            ...
            if (isActive) {
                ...
                creatures = GetCreatureListInRadius(spellCaster.magicSource.position, attractRadius).ToList();
                creatures.Where(creature => !previousCreatures.Contains(creature)).ToList().ForEach(creature => FreezeController.Instance.FreezeCreature(creature));
                previousCreatures.Where(creature => !creatures.Contains(creature)).ToList().ForEach(creature => FreezeController.Instance.UnFreezeCreature(creature));
                previousCreatures = new List<Creature>(creatures);
